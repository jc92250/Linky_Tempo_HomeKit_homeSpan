...
Purpose : teleinfo serial char received processing, should be called
          by main loop, this will take care of managing all the other
...
_State_e TInfo::process(char c)
{
  ...
  switch (c)  {
    ...
    case  TINFO_STX:
      TI_Debugln(F("LibTeleinfo: case TINFO_STX <<<<<<<<<<<<<<<<<<"));
    ...
    //frame  interruption
    case TINFO_EOT:
      TI_Debugln(F("LibTeleinfo: case TINFO_EOT <<<<<<<<<<<<<<<<<<"));
    ...
    // End of group \r ?
    case  TINFO_EGR:
      TI_Debugf(F("LibTeleinfo: case TINFO_EGR _recv_idx=%d"), _recv_idx);
    ...
    default:
    {
      TI_Debugln(F("LibTeleinfo: case other"));
